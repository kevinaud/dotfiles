...
_`set windows-powershell` uses the legacy `powershell.exe` binary, and is no_
_longer recommended. See the `windows-shell` setting above for a more flexible_
_way to control which shell is used on Windows._
...
_[Nushell](https://github.com/nushell/nushell) was written in Rust, and **has**_
_**cross-platform support for Windows / macOS and Linux**._